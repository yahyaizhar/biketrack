    ...
    <div class="row">
        ...
        <!--begin::Portlet-->
            <div class="kt-portlet">
                ...
                <form class="kt-form" action="{{ route('admin.post_bike_rent') }}" id="salik" method="POST" enctype="multipart/form-data">
                    {{ csrf_field() }}
...
<script>
  ...
  $('#salik [name="bike_id"],#salik [name="month"]').on('change', function(){
            ...
            if(_bikeid==''||_month=='')return;
            _month = new Date(_month).format('m');
            console.log(_bikeid, _month);
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }, 
                url:"{{url('admin/store/salik/')}}"+'/'+_bikeid,
                method: "GET"
            })
            .done(function(data) {  
                console.log(data);
