...
//  end overall_balnce_monthly
    ...
    public function zomato_salary_export($month, $client_id)
    {
        ...
        ->addColumn('sim_charges', function($rider) use ($month) {
            $sim_charges=Rider_Account::where('rider_id',$rider->id)
            ->whereNotNull('sim_transaction_id')
            ->whereMonth('month', $month)
            ->get()
            ->sum('amount');
    ...
    public function getEmployeeAccounts($ranges)
    {
        ...
        $from = date($ranges['range']['start_date']);
        $to = date($ranges['range']['end_date']);
        $employee_statements = collect([]);
        $employee_statements_RAW =EmployeeAccounts::where("employee_id",$ranges['empployee_id'])
        ->whereDate('month', '>=',$from)
        ->whereDate('month', '<=',$to)
        ->get();
        foreach ($employee_statements_RAW as $employee_statement) {
            $continue = false;
            if($employee_statement->fuel_expense_id != null ||
            $employee_statement->sim_transaction_id != null ||
            $employee_statement->salik_id != null){
                if($employee_statement->payment_status=="pending" && $employee_statement->type!="cr"){
                    //skip this
                    $continue = true;
                }
            }
            ...
            if(!$continue){
                $employee_statements->push($employee_statement);
            }
        }
        $c_debits_cr_payable =EmployeeAccounts::where("employee_id",$ranges['empployee_id'])
        ->where(function($q) {
            $q->where('type', "dr_receivable")
              ->orWhere('type', 'cr');
        })
        ...
        $c_debits_dr_payable =EmployeeAccounts::where("employee_id",$ranges['empployee_id'])
        ->where('type', 'dr')
        ->sum('amount');
        
        //
        $c_debits_rn_cr_payable =EmployeeAccounts::where("employee_id",$ranges['empployee_id'])
        ->where(function($q) {
            $q->where('type', "dr_receivable")
              ->orWhere('type', 'cr');
        })
        ...
        $c_debits_rn_dr_payable =EmployeeAccounts::where("employee_id",$ranges['empployee_id'])
        ->where('type', 'dr')
        ->whereDate('month', '>=',$from)
        ->whereDate('month', '<=',$to)
        ->sum('amount');

        $c_debits_rn_pl_payable =EmployeeAccounts::where("employee_id",$ranges['empployee_id'])
        ->where('type', 'pl')
        ->whereDate('month', '>=',$from)
        ->whereDate('month', '<=',$to)
        ->sum('amount');
        $running_static_balance = $c_debits_rn_cr_payable - $c_debits_rn_dr_payable - $c_debits_rn_pl_payable;
        //
        $c_debits_rn_pl_total =EmployeeAccounts::where("employee_id",$ranges['empployee_id'])
        ->where('type', 'pl')
        ->whereDate('month', '<',$from)
        ->sum('amount');
        // $profit = $c_debits_rn_pl_total;
        ...
        $c_debits_cr_prev_payable =EmployeeAccounts::where("employee_id",$ranges['empployee_id'])
        ->where(function($q) {
            $q->where('type', "dr_receivable")
              ->orWhere('type', 'cr');
        })
        ...
        $c_debits_dr_prev_payable =EmployeeAccounts::where("employee_id",$ranges['empployee_id'])
        ->where(function($q) {
            $q->where('type', "dr")
            ->orWhere('type', 'pl');
        })
        ...
        $closing_balance = $c_debits_cr_payable - $c_debits_dr_payable;
        $first_month = Carbon::now()->format('Y-m-d');
        if(count($employee_statements) > 0){
            $first_month = $employee_statements->last()->month;
        }
        ...
        $flag = new \App\Model\Accounts\EmployeeAccounts;
        $flag->month='';
        $flag->source='Opening Balance';
        $flag->type='skip';
        $flag->amount=0;
        $employee_statements->prepend($flag);

        $flag = new EmployeeAccounts;
        $flag->month='';
        $flag->source='Closing Balance';
        $flag->type='skip';
        $flag->amount=0;
        $employee_statements->push($flag);
        return DataTables::of($employee_statements)
        ->addColumn('date', function($employee_statements){

            if($employee_statements->type=='skip') return '';
            return Carbon::parse($employee_statements->given_date)->format('M d, Y');
        })
        ->addColumn('desc', function($employee_statement)  use ($employee_statements){

            if($employee_statement->type=='skip') return '<strong >'.$employee_statement->source.'</strong>';
            return $employee_statement->source;
        })
        ->addColumn('cr', function($employee_statements){

            if($employee_statements->type=='pl') return 0;
            if($employee_statements->type=='skip') return '';
            if ($employee_statements->type=='cr' || $employee_statements->type=='dr_receivable')
            {
                return '<span >'.$employee_statements->amount.'</span>';
            }
            return 0;
        })
        ->addColumn('dr', function($employee_statements){

            if($employee_statements->type=='pl') return 0;
            if($employee_statements->type=='skip') return '';
            if($employee_statements->type=='dr'){
                return '<span>('.$employee_statements->amount.')</span>';
            }
            return 0;
        })
        ->addColumn('company_profit', function($employee_statements) use (&$profit){

            
            if($employee_statements->type=='pl'){
                $profit +=$employee_statements->amount;
                return round($employee_statements->amount, 2);
            }
            if($employee_statements->type=='skip') return '<strong>'.round($profit, 2).'</strong>';
            return 0;
        })
        ->addColumn('balance', function($employee_statements) use (&$running_balance){

            
            if($employee_statements->type=='dr' || $employee_statements->type=='pl'){
                $running_balance -= $employee_statements->amount;
            }
            else{
                $running_balance += $employee_statements->amount;
            }
            $_id = $employee_statements->source=="Closing Balance"? 'running_closing_balance':'running_opening_balance';
            // if($company_statements->type=='pl') return 0;
            if($employee_statements->type=='skip') return '<strong id="'.$_id.'"> '.round($running_balance,2).'</strong>';
            return round($running_balance,2);
        })
    ...
    public function getEmployeeAccountsBills($ranges) 
    {
        $ranges = json_decode($ranges, true);
        $from = date($ranges['range']['start_date']);
        $to = date($ranges['range']['end_date']);
        $employee_id = $ranges['empployee_id'];

        $bills = collect([]);

        $month = Carbon::parse($to)->format('m');
        //sim
        $modelArr =EmployeeAccounts::
        whereMonth('month', $month)
