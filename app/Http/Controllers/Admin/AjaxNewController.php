    ...
    public function zomato_septemmber_sheet(){
        ...
        $client_riders=Client_Rider::where('client_id', $zomato->id)->get();
            ...
            ->addColumn('payouts', function($riders){
                ...
                ->get();
                foreach ($number_of_hours_sum as $hour) {
                    $hours=$hour->log_in_hours_payable;
            ...
            ->addColumn('kingrider_salaries', function($rider){
                ...
                $ra_salary=round($total_salary + $ra_cr,2);
                $ra_recieved=round($ra_salary - $ra_payable,2);

                $kingriders_salaries='<div>Net: <strong>'. $ra_salary.'</strong></div><div>Gross: <strong>'. $ra_recieved.'</strong></div>';
                return  $kingriders_salaries;
            })
            ...
            ->addColumn('bike_rent', function($riders){
                ...
                foreach ($zomato as $item) {
                    $key_found = null;
                    $zp_found = Arr::first($date_arr, function ($item_zp, $key) use ($riders, &$key_found){
                        $key_found = $key;
                        return $item_zp['feid'] == $riders->client_rider_id;
                    });
                   if (isset($zp_found)) {
                        $date_arr[$key_found]['count'] = ++$date_arr[$key_found]['count'];
                    }
                    else{
                        $obj=[];
                        $obj['feid']=$item['feid'];
